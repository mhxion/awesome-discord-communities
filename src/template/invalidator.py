...
class CheckInvalid:
    ...
    def check_invites(self, sleep: int = 1):
        invalid_codes = {}
        with open(self.dump, encoding="utf-8", newline="\n", mode="r") as d:
            invites = json.load(d)
            for k in invites.keys():
                r = metadata.DiscordCommunityMetadata(invite_code=k, sleep=sleep).parse_data()
                try:
                    if not r:
                        invalid_codes[k] = invites[k]
                except KeyError:
                    raise requests.exceptions.Timeout(f'Possible rate limit. Reply: {k} : {r}')
